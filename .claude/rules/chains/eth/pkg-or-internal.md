...
- Code that requires `Ethereum` struct properties (`ethClient`, `conf`, `coinTypeCode`, etc.)
- Business logic that adds config fallbacks or type conversions at the domain boundary

```go
// Good: uses e.ethClient (typed client) directly — ethClient not exposed via pkgrpc
...
| Does it use `e.conf` or `e.ethClient`?                      | `internal/infrastructure/api/eth/eth/`          | ↓    |
