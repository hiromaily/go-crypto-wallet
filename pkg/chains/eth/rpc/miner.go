...
func StartMining(ctx context.Context, caller RPCCaller) error {
	var r []byte
	err := caller.CallContext(ctx, &r, "miner_start")
	if err != nil {
...
func StopMining(ctx context.Context, caller RPCCaller) error {
	err := caller.CallContext(ctx, nil, "miner_stop")
	if err != nil {
...
func Mining(ctx context.Context, caller RPCCaller) (bool, error) {
	var bRet bool
	err := caller.CallContext(ctx, &bRet, "eth_mining")
	if err != nil {
...
func HashRate(ctx context.Context, caller RPCCaller) (*big.Int, error) {
	var hashCount string
	err := caller.CallContext(ctx, &hashCount, "eth_hashrate")
	if err != nil {
