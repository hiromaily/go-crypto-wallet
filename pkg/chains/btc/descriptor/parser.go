...
func (p *DescriptorParser) Parse(descriptorStr string) (*Descriptor, error) {
...
func determineType(descriptorStr string) (DescriptorType, error) {
...
func (p *DescriptorParser) extractKeys(descriptorStr string) ([]DescriptorKey, error) {
	...
	if len(matches) == 0 {
		...
		var keys []DescriptorKey
		for _, xpub := range simpleMatches {
			keys = append(keys, DescriptorKey{
				Fingerprint:    "",
				DerivationPath: "",
				ExtendedPubKey: xpub,
			})
	...
	keys := make([]DescriptorKey, 0, len(matches))
...
func FormatDescriptor(desc *Descriptor) (string, error) {
