// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: btc_account_key.sql

package sqlc

import (
	"context"
	"database/sql"
	"strings"
)

const getBtcAccountKeysByAddrStatus = `-- name: GetBtcAccountKeysByAddrStatus :many
SELECT id, coin, key_type, account, p2pkh_address, p2sh_segwit_address, bech32_address, taproot_address, full_public_key, multisig_address, redeem_script, wallet_import_format, idx, addr_status, updated_at FROM btc_account_key WHERE coin = ? AND account = ? AND addr_status = ?
`

type GetBtcAccountKeysByAddrStatusParams struct {
	Coin       BtcAccountKeyCoin
	Account    BtcAccountKeyAccount
	AddrStatus int8
}

func (q *Queries) GetBtcAccountKeysByAddrStatus(ctx context.Context, arg GetBtcAccountKeysByAddrStatusParams) ([]BtcAccountKey, error) {
	rows, err := q.db.QueryContext(ctx, getBtcAccountKeysByAddrStatus, arg.Coin, arg.Account, arg.AddrStatus)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []BtcAccountKey
	for rows.Next() {
		var i BtcAccountKey
		if err := rows.Scan(
			&i.ID,
			&i.Coin,
			&i.KeyType,
			&i.Account,
			&i.P2pkhAddress,
			&i.P2shSegwitAddress,
			&i.Bech32Address,
			&i.TaprootAddress,
			&i.FullPublicKey,
			&i.MultisigAddress,
			&i.RedeemScript,
			&i.WalletImportFormat,
			&i.Idx,
			&i.AddrStatus,
			&i.UpdatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const getBtcAccountKeysByMultisigAddresses = `-- name: GetBtcAccountKeysByMultisigAddresses :many
SELECT id, coin, key_type, account, p2pkh_address, p2sh_segwit_address, bech32_address, taproot_address, full_public_key, multisig_address, redeem_script, wallet_import_format, idx, addr_status, updated_at FROM btc_account_key WHERE coin = ? AND account = ? AND multisig_address IN (/*SLICE:addrs*/?)
`

type GetBtcAccountKeysByMultisigAddressesParams struct {
	Coin    BtcAccountKeyCoin
	Account BtcAccountKeyAccount
	Addrs   []string
}

func (q *Queries) GetBtcAccountKeysByMultisigAddresses(ctx context.Context, arg GetBtcAccountKeysByMultisigAddressesParams) ([]BtcAccountKey, error) {
	query := getBtcAccountKeysByMultisigAddresses
	var queryParams []interface{}
	queryParams = append(queryParams, arg.Coin)
	queryParams = append(queryParams, arg.Account)
	if len(arg.Addrs) > 0 {
		for _, v := range arg.Addrs {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:addrs*/?", strings.Repeat(",?", len(arg.Addrs))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:addrs*/?", "NULL", 1)
	}
	rows, err := q.db.QueryContext(ctx, query, queryParams...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []BtcAccountKey
	for rows.Next() {
		var i BtcAccountKey
		if err := rows.Scan(
			&i.ID,
			&i.Coin,
			&i.KeyType,
			&i.Account,
			&i.P2pkhAddress,
			&i.P2shSegwitAddress,
			&i.Bech32Address,
			&i.TaprootAddress,
			&i.FullPublicKey,
			&i.MultisigAddress,
			&i.RedeemScript,
			&i.WalletImportFormat,
			&i.Idx,
			&i.AddrStatus,
			&i.UpdatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const getMaxBtcAccountKeyIndex = `-- name: GetMaxBtcAccountKeyIndex :one

SELECT COALESCE(MAX(idx), 0) as max_idx FROM btc_account_key WHERE coin = ? AND account = ?
`

type GetMaxBtcAccountKeyIndexParams struct {
	Coin    BtcAccountKeyCoin
	Account BtcAccountKeyAccount
}

// BTC/BCH Account Key Queries
// Note: This file was renamed from account_key.sql to btc_account_key.sql
func (q *Queries) GetMaxBtcAccountKeyIndex(ctx context.Context, arg GetMaxBtcAccountKeyIndexParams) (interface{}, error) {
	row := q.db.QueryRowContext(ctx, getMaxBtcAccountKeyIndex, arg.Coin, arg.Account)
	var max_idx interface{}
	err := row.Scan(&max_idx)
	return max_idx, err
}

const getOneBtcAccountKeyByMaxID = `-- name: GetOneBtcAccountKeyByMaxID :one
SELECT id, coin, key_type, account, p2pkh_address, p2sh_segwit_address, bech32_address, taproot_address, full_public_key, multisig_address, redeem_script, wallet_import_format, idx, addr_status, updated_at FROM btc_account_key WHERE coin = ? AND account = ? ORDER BY id DESC LIMIT 1
`

type GetOneBtcAccountKeyByMaxIDParams struct {
	Coin    BtcAccountKeyCoin
	Account BtcAccountKeyAccount
}

func (q *Queries) GetOneBtcAccountKeyByMaxID(ctx context.Context, arg GetOneBtcAccountKeyByMaxIDParams) (BtcAccountKey, error) {
	row := q.db.QueryRowContext(ctx, getOneBtcAccountKeyByMaxID, arg.Coin, arg.Account)
	var i BtcAccountKey
	err := row.Scan(
		&i.ID,
		&i.Coin,
		&i.KeyType,
		&i.Account,
		&i.P2pkhAddress,
		&i.P2shSegwitAddress,
		&i.Bech32Address,
		&i.TaprootAddress,
		&i.FullPublicKey,
		&i.MultisigAddress,
		&i.RedeemScript,
		&i.WalletImportFormat,
		&i.Idx,
		&i.AddrStatus,
		&i.UpdatedAt,
	)
	return i, err
}

const insertBtcAccountKey = `-- name: InsertBtcAccountKey :execresult
INSERT INTO btc_account_key (
  coin, key_type, account, p2pkh_address, p2sh_segwit_address, bech32_address, taproot_address,
  full_public_key, multisig_address, redeem_script, wallet_import_format, idx, addr_status
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`

type InsertBtcAccountKeyParams struct {
	Coin               BtcAccountKeyCoin
	KeyType            string
	Account            BtcAccountKeyAccount
	P2pkhAddress       string
	P2shSegwitAddress  string
	Bech32Address      string
	TaprootAddress     sql.NullString
	FullPublicKey      string
	MultisigAddress    string
	RedeemScript       string
	WalletImportFormat string
	Idx                int64
	AddrStatus         int8
}

func (q *Queries) InsertBtcAccountKey(ctx context.Context, arg InsertBtcAccountKeyParams) (sql.Result, error) {
	return q.db.ExecContext(ctx, insertBtcAccountKey,
		arg.Coin,
		arg.KeyType,
		arg.Account,
		arg.P2pkhAddress,
		arg.P2shSegwitAddress,
		arg.Bech32Address,
		arg.TaprootAddress,
		arg.FullPublicKey,
		arg.MultisigAddress,
		arg.RedeemScript,
		arg.WalletImportFormat,
		arg.Idx,
		arg.AddrStatus,
	)
}

const updateBtcAccountKeyAddrStatus = `-- name: UpdateBtcAccountKeyAddrStatus :execresult
UPDATE btc_account_key SET addr_status = ?, updated_at = ?
WHERE coin = ? AND account = ? AND wallet_import_format = ?
`

type UpdateBtcAccountKeyAddrStatusParams struct {
	AddrStatus         int8
	UpdatedAt          sql.NullTime
	Coin               BtcAccountKeyCoin
	Account            BtcAccountKeyAccount
	WalletImportFormat string
}

func (q *Queries) UpdateBtcAccountKeyAddrStatus(ctx context.Context, arg UpdateBtcAccountKeyAddrStatusParams) (sql.Result, error) {
	return q.db.ExecContext(ctx, updateBtcAccountKeyAddrStatus,
		arg.AddrStatus,
		arg.UpdatedAt,
		arg.Coin,
		arg.Account,
		arg.WalletImportFormat,
	)
}

const updateBtcAccountKeyAddress = `-- name: UpdateBtcAccountKeyAddress :execresult
UPDATE btc_account_key SET p2pkh_address = ?, updated_at = ?
WHERE coin = ? AND account = ? AND p2sh_segwit_address = ?
`

type UpdateBtcAccountKeyAddressParams struct {
	P2pkhAddress      string
	UpdatedAt         sql.NullTime
	Coin              BtcAccountKeyCoin
	Account           BtcAccountKeyAccount
	P2shSegwitAddress string
}

func (q *Queries) UpdateBtcAccountKeyAddress(ctx context.Context, arg UpdateBtcAccountKeyAddressParams) (sql.Result, error) {
	return q.db.ExecContext(ctx, updateBtcAccountKeyAddress,
		arg.P2pkhAddress,
		arg.UpdatedAt,
		arg.Coin,
		arg.Account,
		arg.P2shSegwitAddress,
	)
}

const updateBtcAccountKeyMultisigAddr = `-- name: UpdateBtcAccountKeyMultisigAddr :execresult
UPDATE btc_account_key
SET multisig_address = ?, redeem_script = ?, addr_status = ?, updated_at = ?
WHERE coin = ? AND account = ? AND full_public_key = ?
`

type UpdateBtcAccountKeyMultisigAddrParams struct {
	MultisigAddress string
	RedeemScript    string
	AddrStatus      int8
	UpdatedAt       sql.NullTime
	Coin            BtcAccountKeyCoin
	Account         BtcAccountKeyAccount
	FullPublicKey   string
}

func (q *Queries) UpdateBtcAccountKeyMultisigAddr(ctx context.Context, arg UpdateBtcAccountKeyMultisigAddrParams) (sql.Result, error) {
	return q.db.ExecContext(ctx, updateBtcAccountKeyMultisigAddr,
		arg.MultisigAddress,
		arg.RedeemScript,
		arg.AddrStatus,
		arg.UpdatedAt,
		arg.Coin,
		arg.Account,
		arg.FullPublicKey,
	)
}
