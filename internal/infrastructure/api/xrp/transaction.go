...
func (r *XRP) CreateRawTransaction(
	...
	if amount != 0 && (ToFloat64(accountInfo.XrpBalance)-xrpkg.MinimumReserve) <= amount {
	...
	calculatedAmount := ToFloat64(accountInfo.XrpBalance) - xrpkg.MinimumReserve - XRPToDrops(ToFloat64(txJSON.Fee))
	if amount == 0 {
