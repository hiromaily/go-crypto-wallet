...
func (r *XRPDetailTxInputRepositorySqlc) Insert(txItem *domainXrp.XRPDetailTx) error {
	...
	var sentUpdatedAt sql.NullTime
	...
	_, err := r.queries.InsertXRPDetailTx(ctx, sqlcgen.InsertXRPDetailTxParams{
		...
		Fee:                   txItem.Fee,
		Flags:                 int64(txItem.Flags),
		LastLedgerSequence:    int64(txItem.LastLedgerSequence),
		Sequence:              int64(txItem.Sequence),
		SigningPubkey:         txItem.SigningPubkey,
		TxnSignature:          txItem.TxnSignature,
		Hash:                  txItem.Hash,
		EarliestLedgerVersion: int64(txItem.EarliestLedgerVersion),
