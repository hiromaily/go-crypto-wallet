...
- [x] 6.1 Refactor SendTransactionUseCase for JSON validation with error handling
  - [x] Replace CSV parsing with `ReadJSONTransactionFile()`
  - [x] Validate all transactions have `isComplete == true` before submission
  - [x] Verify `signedBlob` is non-null and hex-encoded
  - [x] Extract signed blobs for ledger submission
  - [x] Return descriptive errors for incomplete transactions (insufficient signatures)
  - [x] Propagate XRP Ledger error codes and messages (tefPAST_SEQ, tecUNFUNDED_PAYMENT, etc.)
  - [x] Log file path and error details at ERROR level for parsing failures
  - [x] Handle batch transaction submission (collect successes and errors)
  - _Requirements: 4.1, 4.3, 4.5, 4.6, 10.1, 10.3_

- [x] 6.2 Implement transaction submission via TransactionSubmitter
  - [x] Replace direct xrpl-go calls with `TransactionSubmitter` interface dependency
  - [x] Submit signed transaction blobs to XRP Ledger
  - [x] Return transaction hash and ledger version on success
