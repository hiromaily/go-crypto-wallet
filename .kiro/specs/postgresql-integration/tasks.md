...
- [ ] 2.3 (P) Generate initial PostgreSQL migrations from HCL
  ...
  - **Status**: Migration directories created with README documentation. Docker is available. Atlas CLI v1.1.3 installed.
  - **To complete**: Run `cd tools/atlas && atlas migrate diff initial_schema --env local_postgresql_watch` (and keygen/sign)
  - **Note**: Atlas uses `docker://postgres/18/*` dev databases (ephemeral containers).
    Does NOT require compose `wallet-postgres` service.
...
- [x] 3.1 Configure PostgreSQL database service
  ...
  - Configure volume: wallet-postgres:/var/lib/postgresql/data
