-- Create "address" table
CREATE TABLE "address" (
  "id" bigserial NOT NULL COMMENT "ID",
  "coin" text NOT NULL CHECK ("coin" IN ('btc','bch','eth','xrp','hyt')) COMMENT "coin type code",
  "account" text NOT NULL CHECK ("account" IN ('client','deposit','payment','stored')) COMMENT "account type",
  "wallet_address" varchar(255) NOT NULL COMMENT "wallet address",
  "is_allocated" boolean NOT NULL DEFAULT false COMMENT "true: address is allocated(used)",
  "updated_at" timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT "updated date",
  PRIMARY KEY ("id"),
  INDEX "idx_account" ("account"),
  INDEX "idx_coin" ("coin"),
  UNIQUE INDEX "idx_wallet_address" ("wallet_address")
) COMMENT "table for account pubkey";
-- Create "btc_tx" table
CREATE TABLE "btc_tx" (
  "id" bigserial NOT NULL COMMENT "transaction ID",
  "coin" text NOT NULL CHECK ("coin" IN ('btc','bch')) COMMENT "coin type code",
  "action" text NOT NULL CHECK ("action" IN ('deposit','payment','transfer')) COMMENT "action type",
  "unsigned_hex_tx" text NOT NULL COMMENT "HEX string for unsigned transaction",
  "signed_hex_tx" text NOT NULL COMMENT "HEX string for signed transaction",
  "sent_hash_tx" text NOT NULL COMMENT "Hash for sent transaction",
  "total_input_amount" numeric(26,10) NOT NULL COMMENT "total amount of coin to send",
  "total_output_amount" numeric(26,10) NOT NULL COMMENT "total amount of coin to receive without fee",
  "fee" numeric(26,10) NOT NULL COMMENT "fee",
  "current_tx_type" smallint NOT NULL DEFAULT 1 COMMENT "current transaction type",
  "unsigned_updated_at" timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT "updated date for unsigned transaction created",
  "sent_updated_at" timestamp NULL COMMENT "updated date for signed transaction sent",
  PRIMARY KEY ("id"),
  INDEX "idx_action" ("action"),
  INDEX "idx_coin" ("coin")
) COMMENT "table for btc transaction info";
-- Create "btc_tx_input" table
CREATE TABLE "btc_tx_input" (
  "id" bigserial NOT NULL COMMENT "ID",
  "tx_id" bigint NOT NULL COMMENT "tx table ID",
  "input_txid" varchar(255) NOT NULL COMMENT "txid for input",
  "input_vout" integer NOT NULL COMMENT "vout for input",
  "input_address" varchar(255) NOT NULL COMMENT "sender address for input",
  "input_account" varchar(255) NOT NULL COMMENT "sender account for input",
  "input_amount" numeric(26,10) NOT NULL COMMENT "amount of coin to send for input",
  "input_confirmations" bigint NOT NULL COMMENT "block confirmations when unspent rpc returned",
  "updated_at" timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT "updated date",
  PRIMARY KEY ("id"),
  INDEX "idx_tx_id" ("tx_id")
) COMMENT "table for input transaction";
-- Create "btc_tx_output" table
CREATE TABLE "btc_tx_output" (
  "id" bigserial NOT NULL COMMENT "ID",
  "tx_id" bigint NOT NULL COMMENT "tx table ID",
  "output_address" varchar(255) NOT NULL COMMENT "receiver address for output",
  "output_account" varchar(255) NOT NULL COMMENT "receiver account for output",
  "output_amount" numeric(26,10) NOT NULL COMMENT "amount of coin to receive",
  "is_change" boolean NOT NULL DEFAULT false COMMENT "true: output is for fee",
  "updated_at" timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT "updated date",
  PRIMARY KEY ("id"),
  INDEX "idx_tx_id" ("tx_id")
) COMMENT "table for output transaction";
-- Create "eth_detail_tx" table
CREATE TABLE "eth_detail_tx" (
  "id" bigserial NOT NULL COMMENT "ID",
  "tx_id" bigint NOT NULL COMMENT "eth_tx table ID",
  "uuid" varchar(36) NOT NULL COMMENT "UUID",
  "current_tx_type" smallint NOT NULL DEFAULT 1 COMMENT "current transaction type",
  "sender_account" varchar(255) NOT NULL COMMENT "sender account",
  "sender_address" varchar(255) NOT NULL COMMENT "sender address",
  "receiver_account" varchar(255) NOT NULL COMMENT "receiver account",
  "receiver_address" varchar(255) NOT NULL COMMENT "receiver address",
  "amount" bigint NOT NULL COMMENT "amount of coin to receive",
  "fee" bigint NOT NULL COMMENT "fee",
  "gas_limit" integer NOT NULL COMMENT "gas limit",
  "nonce" bigint NOT NULL COMMENT "nonce",
  "unsigned_hex_tx" text NOT NULL COMMENT "HEX string for unsigned transaction",
  "signed_hex_tx" text NOT NULL COMMENT "HEX string for signed transaction",
  "sent_hash_tx" text NOT NULL COMMENT "Hash for sent transaction",
  "unsigned_updated_at" timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT "updated date for unsigned transaction created",
  "sent_updated_at" timestamp NULL COMMENT "updated date for signed transaction sent",
  PRIMARY KEY ("id"),
  INDEX "idx_receiver_account" ("receiver_account"),
  INDEX "idx_sender_account" ("sender_account"),
  INDEX "idx_txid" ("tx_id"),
  UNIQUE INDEX "idx_uuid" ("uuid")
) COMMENT "table for eth transaction detail";
-- Create "payment_request" table
CREATE TABLE "payment_request" (
  "id" bigserial NOT NULL COMMENT "ID",
  "coin" text NOT NULL CHECK ("coin" IN ('btc','bch','eth','xrp')) COMMENT "coin type code",
  "payment_id" bigint NULL COMMENT "tx table ID for payment action",
  "sender_address" varchar(255) NOT NULL COMMENT "sender address",
  "sender_account" varchar(255) NOT NULL COMMENT "sender account",
  "receiver_address" varchar(255) NOT NULL COMMENT "receiver address",
  "amount" numeric(26,10) NOT NULL COMMENT "amount of coin to send",
  "is_done" boolean NOT NULL DEFAULT false COMMENT "true: unsigned transaction is created",
  "updated_at" timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT "updated date",
  PRIMARY KEY ("id"),
  INDEX "idx_coin" ("coin")
) COMMENT "table for payment request";
-- Create "tx" table
CREATE TABLE "tx" (
  "id" bigserial NOT NULL COMMENT "transaction ID",
  "coin" text NOT NULL CHECK ("coin" IN ('eth','xrp','hyt')) COMMENT "coin type code",
  "action" text NOT NULL CHECK ("action" IN ('deposit','payment','transfer')) COMMENT "action type",
  "updated_at" timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT "updated date",
  PRIMARY KEY ("id"),
  INDEX "idx_action" ("action"),
  INDEX "idx_coin" ("coin")
) COMMENT "table for eth transaction info";
-- Create "xrp_detail_tx" table
CREATE TABLE "xrp_detail_tx" (
  "id" bigserial NOT NULL COMMENT "ID",
  "tx_id" bigint NOT NULL COMMENT "xrp_tx table ID",
  "uuid" varchar(36) NOT NULL COMMENT "UUID",
  "current_tx_type" smallint NOT NULL DEFAULT 1 COMMENT "current transaction type",
  "sender_account" varchar(255) NOT NULL COMMENT "sender account",
  "sender_address" varchar(255) NOT NULL COMMENT "sender address",
  "receiver_account" varchar(255) NOT NULL COMMENT "receiver account",
  "receiver_address" varchar(255) NOT NULL COMMENT "receiver address",
  "amount" varchar(255) NOT NULL COMMENT "amount of coin to receive",
  "xrp_tx_type" varchar(255) NOT NULL COMMENT "xrp tx type like "Payment"",
  "fee" varchar(255) NOT NULL COMMENT "tx fee",
  "flags" bigint NOT NULL COMMENT "tx flags",
  "last_ledger_sequence" bigint NOT NULL COMMENT "tx LastLedgerSequence",
  "sequence" bigint NOT NULL COMMENT "tx Sequence",
  "signing_pubkey" varchar(255) NOT NULL COMMENT "tx SigningPubKey",
  "txn_signature" varchar(255) NOT NULL COMMENT "tx TxnSignature",
  "hash" varchar(255) NOT NULL COMMENT "tx Hash",
  "earliest_ledger_version" bigint NOT NULL COMMENT "tx earliest_ledger_version after sending tx",
  "signed_tx_id" varchar(255) NOT NULL COMMENT "signed tx id",
  "tx_blob" text NOT NULL COMMENT "sent tx blob",
  "sent_updated_at" timestamp NULL COMMENT "updated date for signed transaction sent",
  PRIMARY KEY ("id"),
  INDEX "idx_receiver_account" ("receiver_account"),
  INDEX "idx_sender_account" ("sender_account"),
  INDEX "idx_txid" ("tx_id"),
  UNIQUE INDEX "idx_uuid" ("uuid")
) COMMENT "table for xrp transaction detail";
-- Create "xrp_multisig_signature" table
CREATE TABLE "xrp_multisig_signature" (
  "id" bigserial NOT NULL COMMENT "ID",
  "pending_multisig_id" bigint NOT NULL COMMENT "Reference to xrp_pending_multisig.id",
  "signer_account" varchar(255) NOT NULL COMMENT "XRP address of the signer (r...)",
  "signed_tx_blob" text NOT NULL COMMENT "Signed transaction blob from this signer",
  "signer_weight" integer NOT NULL COMMENT "Weight of this signature",
  "signed_at" timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT "date when signature was collected",
  PRIMARY KEY ("id"),
  INDEX "idx_pending_multisig_id" ("pending_multisig_id"),
  UNIQUE INDEX "idx_pending_signer" ("pending_multisig_id", "signer_account") COMMENT "Each signer can only sign once per transaction",
  INDEX "idx_signer_account" ("signer_account")
) COMMENT "Collected signatures for XRP multi-signature transactions";
-- Create "xrp_pending_multisig" table
CREATE TABLE "xrp_pending_multisig" (
  "id" bigserial NOT NULL COMMENT "ID",
  "tx_uuid" varchar(36) NOT NULL COMMENT "Unique identifier for the transaction",
  "account_id" varchar(255) NOT NULL COMMENT "XRP account address (r...) that will sign this transaction",
  "unsigned_tx_json" text NOT NULL COMMENT "JSON representation of the unsigned transaction",
  "xrp_tx_type" varchar(50) NOT NULL COMMENT "XRP transaction type (Payment, SignerListSet, etc.)",
  "required_quorum" integer NOT NULL COMMENT "Total signature weight required for this transaction",
  "current_weight" integer NOT NULL DEFAULT false COMMENT "Current total weight of collected signatures",
  "status" text NOT NULL CHECK ("status" IN ('pending','ready','submitted','confirmed','failed','expired')) DEFAULT "pending" COMMENT "Current status of the multi-sig transaction",
  "combined_tx_blob" text NULL COMMENT "Combined signed transaction blob (set when status is ready)",
  "submitted_tx_hash" varchar(255) NULL COMMENT "Transaction hash after submission to ledger",
  "expires_at" timestamp NULL COMMENT "Expiration time for this pending transaction",
  "created_at" timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT "creation date",
  "updated_at" timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT "last update date",
  PRIMARY KEY ("id"),
  INDEX "idx_account_id" ("account_id"),
  INDEX "idx_account_status" ("account_id", "status") COMMENT "Find pending transactions for an account",
  INDEX "idx_status" ("status"),
  UNIQUE INDEX "idx_tx_uuid" ("tx_uuid")
) COMMENT "Pending XRP multi-signature transactions awaiting signatures";
