...
`wallet / keygen / sign` の3つのDBを、**PostgreSQL / MySQL / SQLite** のいずれか（または複数）に対して運用するフローを **mermaid** で整理したもの。
...
flowchart TD
  ...
  B --> C[atlas.hcl を用意\nenv を DBと方言ごとに定義\n例: pg_wallet, mysql_wallet, sqlite_wallet]
  C --> D{対象DBはどれ?}
...
- 3 DB×3 dialectを全部カバーするなら、運用上は **migrations を dialect/DBごとに分ける**のが事故りにくい（e.g. `migrations/postgres/wallet` など）。
