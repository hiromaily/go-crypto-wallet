...
# Bitcoin E2E Workflow Script - Pattern 9: P2TR Taproot Single-sig
...
#
# Transaction Pattern:
#   Pattern 9: BTC P2TR Taproot Single-sig
#   - Address Type: P2TR (BIP86 Taproot)
#   - Address Format: `bc1p...` (Mainnet), `tb1p...` (Testnet), `bcrt1p...` (Regtest)
#   - Signature Requirement: Single-sig (Keygen only, Schnorr signature)
...
# Use single-sig account configuration for Pattern 9
...
#
# Pattern 9 (P2TR Taproot Single-sig) requires:
...
key_generation_phase() {
	...
	# NOTE: Private key import is NOT needed for descriptor-based wallets (Pattern 9)
...
singlesig_setup_phase() {
	...
	# For Pattern 9 (P2TR Taproot Single-sig), use account.yaml
	...
	log_info "Note: Pattern 9 uses account.yaml (all accounts are single-sig)"
	log_info "Note: P2TR provides Taproot with Bech32m encoding and Schnorr signatures"
	...
	# Extract first P2TR descriptor from payment_descriptors.json
	# For P2TR (BIP86), we use the descriptor at the appropriate index
...
create_payment_requests_phase() {
	...
	# For P2TR testing, query for Bech32m addresses (starting with 'bcrt1p' in regtest)
	...
	if [[ "$sender_address" =~ ^bcrt1p ]]; then
		log_info "✓ Address format verified: P2TR Taproot (regtest)"
	else
		log_warn "⚠ Unexpected address format: expected 'bcrt1p...' for P2TR regtest, got: $sender_address"
	fi
...
This script automates the complete Bitcoin workflow for single-signature P2TR
(Taproot) transactions. It serves as a regression test tool to verify that the
Bitcoin single-sig Taproot workflow functions correctly after code changes.
...
The script performs the following steps:
 ...
 10. Sign with keygen wallet (single Schnorr signature)
 11. Broadcast transaction

The script uses descriptor-based import for P2TR single-sig addresses,
...
Transaction Pattern Details:
  - Address Type: P2TR (BIP86 Taproot)
  - Address Format: bcrt1p... (Regtest)
  - Signature Requirement: Single-sig (Keygen only)
  - Signature Algorithm: Schnorr (BIP340, 64 bytes)
  - No sign wallets needed
  - Most efficient transaction format with Bech32m encoding
...
main() {
	...
	log_info "Starting Bitcoin E2E Workflow - Pattern 9: P2TR Taproot Single-sig"
	log_info "Coin: $COIN"
	...
	log_info "  ✓ P2TR Taproot single-sig addresses created"
	log_info "  ✓ Test UTXOs generated"
	log_info "  ✓ Payment requests created (using payment account)"
	log_info "  ✓ Transaction created, signed (1 Schnorr signature), and sent"
	echo ""
	log_info "Transaction Pattern Used:"
	log_info "  • P2TR (BIP86 Taproot) single-signature"
	...
	log_info "  • Most efficient transaction format with Bech32m encoding"
	log_info "  • Schnorr signatures (64 bytes, BIP340)"
	echo ""
	log_info "You can now use the wallet system for Bitcoin single-sig Taproot operations"
