...
# This script automates the complete Bitcoin workflow for single-sig P2WPKH (Native SegWit) transactions
...
# Use single-sig account configuration for Pattern 5
...
#
# Pattern 5 (P2WPKH Native SegWit Single-sig) requires:
...
key_generation_phase() {
	...
	# NOTE: Private key import is NOT needed for descriptor-based wallets (Pattern 5)
...
singlesig_setup_phase() {
	...
	# For Pattern 5 (P2WPKH Native SegWit Single-sig), use account.yaml
	...
	log_info "Note: Pattern 5 uses account.yaml (all accounts are single-sig)"
	log_info "Note: P2WPKH provides native SegWit with Bech32 encoding"
	...
	# Extract first P2WPKH descriptor from payment_descriptors.json
	# For P2WPKH (BIP84), we use the descriptor at the appropriate index
...
generate_test_utxos() {
	...
	# Verify address format (should start with 'bcrt1q' for regtest P2WPKH)
...
create_payment_requests_phase() {
	...
	# For P2WPKH testing, query for Bech32 addresses (starting with 'bcrt1q' in regtest)
	...
	if [[ "$sender_address" =~ ^bcrt1q ]]; then
		log_info "✓ Address format verified: P2WPKH Native SegWit (regtest)"
	else
		log_warn "⚠ Unexpected address format: expected 'bcrt1q...' for P2WPKH regtest, got: $sender_address"
	fi
...
This script automates the complete Bitcoin workflow for single-signature P2WPKH
(Native SegWit) transactions. It serves as a regression test tool to verify that the
Bitcoin single-sig Native SegWit workflow functions correctly after code changes.
...
The script uses descriptor-based import for P2WPKH single-sig addresses,
...
Transaction Pattern Details:
  - Address Type: P2WPKH (BIP84 Native SegWit)
  - Address Format: bcrt1q... (Regtest)
  - Signature Requirement: Single-sig (Keygen only)
  - No sign wallets needed
  - Most efficient SegWit v0 with Bech32 encoding
...
main() {
	...
	log_info "  ✓ P2WPKH Native SegWit single-sig addresses created"
	...
	log_info "  • P2WPKH (BIP84 Native SegWit) single-signature"
	...
	log_info "  • Most efficient SegWit v0 with Bech32 encoding"
	echo ""
	log_info "You can now use the wallet system for Bitcoin single-sig Native SegWit operations"
