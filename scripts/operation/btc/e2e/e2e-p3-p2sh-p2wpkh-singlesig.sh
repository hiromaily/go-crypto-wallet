...
#
# Pattern 3 (P2SH-P2WPKH Single-sig) requires:
...
key_generation_phase() {
	...
	# NOTE: Private key import is NOT needed for descriptor-based wallets (Pattern 3)
...
singlesig_setup_phase() {
	...
	# For Pattern 3 (P2SH-P2WPKH Single-sig), use account.yaml
	...
	log_info "Note: Pattern 3 uses account.yaml (all accounts are single-sig)"
	log_info "Note: P2SH-P2WPKH provides SegWit benefits with backward compatibility"
	...
	# Extract first P2SH-P2WPKH descriptor from payment_descriptors.json
	# For P2SH-P2WPKH (BIP49), we use the descriptor at the appropriate index
...
create_payment_requests_phase() {
	...
	# For P2SH-P2WPKH testing, query for P2SH addresses (starting with '2' in regtest)
	...
	if [[ "$sender_address" =~ ^2 ]]; then
		log_info "✓ Address format verified: P2SH (regtest)"
	else
		log_warn "⚠ Unexpected address format: expected '2...' for P2SH regtest, got: $sender_address"
	fi
...
This script automates the complete Bitcoin workflow for single-signature P2SH-P2WPKH
(Nested SegWit) transactions. It serves as a regression test tool to verify that the
Bitcoin single-sig Nested SegWit workflow functions correctly after code changes.
...
The script uses descriptor-based import for P2SH-P2WPKH single-sig addresses,
...
main() {
	...
	log_info "  ✓ P2SH-P2WPKH single-sig addresses created"
	...
	log_info "  • P2SH-P2WPKH (BIP49 Nested SegWit) single-signature"
	...
	echo ""
	log_info "You can now use the wallet system for Bitcoin single-sig Nested SegWit operations"
