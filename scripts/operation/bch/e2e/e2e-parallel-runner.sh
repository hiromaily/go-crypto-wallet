...
# Bitcoin Cash E2E Parallel Test Runner
# This script runs all BCH E2E test patterns (P1-P3) in parallel using SQLite backend
# Usage: ./scripts/operation/bch/e2e/e2e-parallel-runner.sh [OPTIONS]
#
# Options:
...
#
# Description:
#   This script is designed for CI/CD environments to run BCH E2E tests in parallel.
...
#   ./scripts/operation/bch/e2e/e2e-parallel-runner.sh --patterns 1-3 --max-parallel 2
...
declare -A PATTERN_SCRIPTS=(
	...
	[2]="e2e-p2-p2sh-2of3.sh"
	[3]="e2e-p3-p2sh-3of3.sh"
)

# Pattern descriptions
declare -A PATTERN_DESCRIPTIONS=(
	[1]="P2PKH Single-sig"
	[2]="P2SH 2-of-3 Multisig"
	[3]="P2SH 3-of-3 Multisig"
)
...
Examples:
  ...
  $0 --patterns 1-3 --max-parallel 2 --ci
...
# Parse pattern range (e.g., "1-3" or "1,2,3")
parse_patterns() {
...
validate_pattern() {
	...
	if [[ ! "${PATTERN_SCRIPTS[$pattern]+_}" ]]; then
		log_error "Invalid pattern: P=$pattern. Valid patterns are 1-3."
		return 1
	fi
	return 0
}
...
setup_shared_infrastructure() {
	...
	docker compose -f "${PROJECT_ROOT}/compose.bch.yaml" down -v 2>/dev/null || true
	...
	rm -rf "${PROJECT_ROOT}/data/sqlite/bch"/*-e2e-*.db 2>/dev/null || true

	# Clean Bitcoin Cash Node data directories completely for fresh state
	log_info "Cleaning Bitcoin Cash Node data directories..."
	for node in watch keygen sign1 sign2; do
		...
		rm -rf "${PROJECT_ROOT}/docker/nodes/bch/${node}/regtest" 2>/dev/null || true
	done

	# Start Bitcoin Cash node containers (shared across all patterns)
	log_info "Starting shared Bitcoin Cash node containers..."
	log_substep "Starting Bitcoin Cash node containers"
	docker compose -f "${PROJECT_ROOT}/compose.bch.yaml" up -d bch-watch bch-keygen bch-sign1 bch-sign2

	# Wait for containers to be healthy
	log_substep "Waiting for containers to be healthy"
	for container in bch-watch bch-keygen bch-sign1 bch-sign2; do
		wait_for_healthy "$container" 90
	done

	# Additional wait to ensure Bitcoin Cash Node is fully initialized
	log_info "Waiting for Bitcoin Cash Node to fully initialize..."
	sleep 5
...
cleanup_shared_infrastructure() {
	...
	# Clean Bitcoin Cash Node data directories
	log_info "Cleaning Bitcoin Cash Node data directories..."
	for node in watch keygen sign1 sign2; do
		log_info "  Cleaning ${node} regtest directory..."
		rm -rf "${PROJECT_ROOT}/docker/nodes/bch/${node}/regtest" 2>/dev/null || true
...
main() {
	...
	log_info "  Infrastructure: Shared Bitcoin Cash nodes"
	echo ""

	# Setup trap to ensure cleanup happens
	trap cleanup_shared_infrastructure EXIT

	# Setup shared infrastructure (Bitcoin Cash containers)
	setup_shared_infrastructure
