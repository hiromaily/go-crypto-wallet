...
# Bitcoin Cash E2E Workflow Script
# This script automates the complete Bitcoin Cash workflow from infrastructure setup to transaction execution
# Usage: ./scripts/operation/bch/e2e-workflow.sh [OPTIONS]
# Options:
...
setup_infrastructure() {
    ...
    docker compose -f compose.bch.yaml up -d
    log_info "Bitcoin Cash node containers started"

    # Wait for containers to be healthy
    log_substep "Waiting for containers to be healthy"
    wait_for_healthy "bch-watch" 90
    wait_for_healthy "bch-keygen" 90
    wait_for_healthy "bch-sign1" 90
    wait_for_healthy "bch-sign2" 90
...
setup_wallets() {
    ...
    # Create wallets in Bitcoin Cash nodes
    bch_create_wallet_if_needed "bch-watch" "watch"
    bch_create_wallet_if_needed "bch-keygen" "keygen"
    bch_create_wallet_if_needed "bch-sign1" "sign1"
    bch_create_wallet_if_needed "bch-sign2" "sign2"
...
generate_test_utxos() {
    ...
    # CSV format: coin,account,P2PKH,P2SH-segwit,bech32,taproot,pubkey,multisigAddress,index
    # Payment account is a multisig account, so use field 8 (multisig address)
    payment_address=$(grep -v '^#' "${address_payment}" 2>/dev/null | head -n1 | cut -d',' -f8)
    ...
    bch_cli "bch-watch" generatetoaddress 101 "$payment_address" >/dev/null
...
This script automates the complete Bitcoin Cash workflow from infrastructure setup
to transaction execution. It serves as a regression test tool to verify that
the Bitcoin Cash workflow functions correctly after code changes.
...
main() {
    ...
    log_info "Starting Bitcoin Cash E2E Workflow"
    log_info "Coin: $COIN"
    ...
    echo ""
    log_info "You can now use the wallet system for Bitcoin Cash operations"
