...
if [ $CLIENT -eq 1 ]; then
	...
	docker compose exec wallet-mysql mysql -u root -proot keygen -e "$(cat ./docker/mysql/insert/ganache.example.sql)"
else
	# create seed
...
echo $file_address_client
echo ${file_address_client##*\[fileName\]: }
###############################################################################
# watch only wallet
###############################################################################
