#!/bin/bash
# Extract CREATE TABLE statements from PostgreSQL dump files for sqlc schema files
#
# Usage:
#   ./scripts/db/extract-sqlc-schema-postgresql.sh <schema_name> <dump_file> <output_file>
#
# Example:
#   ./scripts/db/extract-sqlc-schema-postgresql.sh watch data/dump/sql/dump_watch.pg.sql tools/sqlc/schemas/postgresql/01_watch.sql
...
if [ $# -ne 3 ]; then
	...
	echo "" >&2
	echo "Extracts CREATE TABLE statements from PostgreSQL dump file and formats them for sqlc." >&2
	echo "" >&2
	echo "Arguments:" >&2
...
echo "-- Code generated by \`make extract-sqlc-schema-postgresql-${SCHEMA_NAME}\`. DO NOT EDIT." >"$OUTPUT_FILE"
